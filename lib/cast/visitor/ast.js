...
class ConventionalCommitVisitor extends BaseCstVisitor {
  ...
  conventionalHeader(ctx) {
    ...
    if (ctx.COLON) {
      const token = ctx.COLON[0]
      children.push(
        node.text({value: token.image, postiion: tokenToPosition(token)})
      )
    }
  ...
  trailer(ctx) {
    ...
    const value = this.visit(ctx.trailervalue)
    ...
    , node.trailerValue({}, value.children)
    ])
  }
  ...
  trailerkey(ctx) {
    ...
    if (ctx.BREAKING_CHANGE) {
      return {token: ctx.BREAKING_CHANGE[0], breaking: true}
    }
  ...
  referenceaction(ctx) {
    const token = ctx.REFERENCE_ACTION[0]
  ...
  reporeference(ctx) {
    ...
    const value = repo + prefix + id_value
    ...
    , owner: null
    , repository: repo
    , position: tokensToPosition([ctx.REPO_AND_PREFIX[0], id_token])
    }, [node.text({value})])
  }
  ...
  remotereference(ctx) {
    ...
    , repository: repo
    , position: tokensToPosition([org_token, prefix_token, id_token])
    }, [node.text({value})])
  }
