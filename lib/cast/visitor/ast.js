...
class ConventionalCommitVisitor extends BaseCstVisitor {
  ...
  commit(ctx) {
    ...
    // Visit footer if present
    if (ctx.footer) {
      const footer = this.visit(ctx.footer)
  ...
  header(ctx) {
    ...
    if (ctx.conventionalHeader) return this.visit(ctx.conventionalHeader)

    // Non-conventional commit: just description
    if (ctx.description) return node.header({}, [this.visit(ctx.description)])
  ...
  conventionalHeader(ctx) {
    ...
    if (ctx.type) children.push(this.visit(ctx.type))
    if (ctx.scope) children.push(this.visit(ctx.scope))
    if (ctx.BANG) {
    ...
    if (ctx.description) children.push(this.visit(ctx.description, has_breaking))
  ...
  footer(ctx) {
    ...
    return node.footer({}, ctx.trailer.map(visit))
  }

  trailer(ctx) {
    // Visit trailer key to determine if this trailer is a breaking change
    const key_result = this.visit(ctx.trailerkey)
    const breaking = !!key_result.breaking

    if (key_result.referenceAction) console.dir(key_result, {depth: 10})
    return node.trailer({breaking: breaking}, [
      node.trailerKey({}, [
        node.text({
  ...
  trailerkey(ctx) {
    ...
    if (ctx.referenceaction) {
      return this.visit(ctx.referenceaction)
    }
  ...
  referenceaction(ctx) {
    const token = ctx.REFERENCE_ACTION_TOKEN[0]
    return {token: token, referenceAction: true}
  }
  ...
  trailervalueline(ctx) {
    // Trailer value line can be either issues or metadata
    if (ctx.issues) return this.visit(ctx.issues)
    if (ctx.metadata) return this.visit(ctx.metadata)
    return []
  }

  issues(ctx) {
    const visit = this.visit.bind(this)
    return ctx.issuereference.map(visit)
  }

  // Parse issue references from CST (e.g., #123, GH-456, JIRA-ABC123)
  issuereference(ctx) {
    if (ctx.localreference) return this.visit(ctx.localreference)
    if (ctx.reporeference) return this.visit(ctx.reporeference)
  ...
  reporeference(ctx) {
    const [repo_token, id_token] = ctx.identifier
    const prefix_token = ctx.ISSUE_PREFIX[0]

    const prefix = prefix_token.image
    const id_value = id_token.image
    const value = repo_token.image + prefix + id_value
    ...
    , position: tokensToPosition([repo_token, prefix_token, id_token])
    }, [node.text({value})])
  }
  ...
  remotereference(ctx) {
    ...
    return node.issuereference({
      value: value
    , prefix: prefix
    , id: typecast(typecast(id_value))
    , owner: org_token.image
    , repository: repo_token.image
    , position: tokensToPosition([org_token, repo_token, prefix_token, id_token])
    }, [node.text({value})])
  }
