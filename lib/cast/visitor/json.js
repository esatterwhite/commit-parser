...
class JSONVisitor extends ASTVisitor {
  ...
  trailerkey(ctx) {
    if (ctx.BREAKING_CHANGE_TOKEN) return {token: ctx.BREAKING_CHANGE_TOKEN[0], breaking: true}
    if (ctx.referenceaction) return this.visit(ctx.referenceaction)
  ...
  reporeference(ctx, output) {
    ...
    return {value, offset: repo_token.startOffset}
  }
  ...
  metadata(ctx, output) {
    ...
    if (ctx.issuereference) {
      for (const ref of ctx.issuereference) {
        const value = this.visit(ref, output)
        items.push(value)
      }
    }
