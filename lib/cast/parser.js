...
class ConventionalCommitParser extends CstParser {
  ...
  , tokens
    ...
    const token_vocabulary = tokens || createTokenVocabulary(config)
    // Convert object to array for Chevrotain (V8 maintains insertion order)
    super(Object.values(token_vocabulary))

    // Extract token references directly from vocabulary object
    const {
      BREAKING_CHANGE_TOKEN
    , REFERENCE_ACTION_TOKEN
    , ISSUE_PREFIX
    } = token_vocabulary
    ...
    this.RULE('issuereference', () => {
      ...
      , {ALT: () => {
          return this.SUBRULE(this.reporeference)
        }}
      , {ALT: () => {
    ...
    // Issue reference from a different repo (skyring#1)
    this.RULE('reporeference', () => {
      this.CONSUME(IDENTIFIER)
      this.CONSUME(ISSUE_PREFIX)
    ...
    this.RULE('trailerkey', () => {
      ...
      , {ALT: () => {
          return this.SUBRULE(this.referenceaction)
        }}
      , {ALT: () => {
          return this.CONSUME(FOOTER_TOKEN)
        }}
    ...
    this.RULE('referenceaction', () => {
      this.CONSUME(REFERENCE_ACTION_TOKEN)
    })
    ...
    this.RULE('issues', () => {
      ...
      , GATE: () => {
          ...
          const POS_ONE = this.LA(pos)
          const POS_TWO = this.LA(pos + 1)
          const POS_THREE = this.LA(pos + 2)
          const POS_FOUR = this.LA(pos + 3)

          {
            const repo = POS_ONE?.tokenType
            const prefix = POS_TWO?.tokenType
            if (repo === IDENTIFIER && prefix === ISSUE_PREFIX) return true
          }

          {
            const org = POS_ONE?.tokenType
            const sep = POS_TWO?.tokenType
            const repo = POS_THREE?.tokenType
            const prefix = POS_FOUR?.tokenType
  ...
  hasColonAfterType() {
    ...
    // FOOTER_TOKEN is included because the lexer matches words like "feat"
    // as FOOTER_TOKEN when followed by a colon (due to pattern lookahead)
    const is_text = this.LA(pos).tokenType === TEXT
    const is_footer = this.LA(pos).tokenType === FOOTER_TOKEN
    if (!is_text && !is_footer) {
      return false
    }
    pos++
