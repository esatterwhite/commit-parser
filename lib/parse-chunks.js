...
const {createRoot} = require('./ast-nodes.js')
...
/**
 * Create tokens for header parsing (includes footer tokens for type matching but excludes breaking change token and body text)
 * @param {Object} config - Configuration options
 * @returns {Array} Array of tokens for header parsing
 */
function createHeaderTokens(config = {notesPhrase: BREAKING_CHANGES}) {
  // Get all tokens but filter out BREAKING_CHANGE_TOKEN and BODY_TEXT for header parsing
  // BODY_TEXT would match structural tokens like "feat!:" as a single token, preventing proper parsing
  const allTokens = createConfigurableTokens(config)
  return allTokens.filter((token) => {
    return token.name !== 'BREAKING_CHANGE_TOKEN' && token.name !== 'BODY_TEXT'
  })
}
...
function createBodyTokens() {
  ...
  , NEW_LINE
  , WHITE_SPACE
  , BODY_TEXT // Permissive text token (allows ()!: characters) - replaces TEXT in body context
  ]
}
 ...
 */
function parseCommitMessage(message, config = {notesPhrase: BREAKING_CHANGES}) {
  ...
  // Parse footer if present using parser's token array (excluding BODY_TEXT)
  if (chunks.footer) {
    // Filter out BODY_TEXT from footer tokens
    const footer_tokens = parser.tokens.filter((token) => {
      return token.name !== 'BODY_TEXT'
    })
    ...
    // Visit the footers CST node directly (not the commit wrapper)
    const footer_ast = visitor.visit(footer_cst.children.footers[0])
