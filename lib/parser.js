...
, issuePrefix: ISSUE_PREFIXES
  ...
  , ISSUE_ID // Issue ID token (must come before LINE_TEXT for issue parsing)
...
class ConventionalCommitParser extends CstParser {
  constructor(config = {notesPhrase: BREAKING_CHANGES, issuePrefix: ISSUE_PREFIXES}) {
    ...
    this.issuePrefixToken = configurableTokens.find((token) => {
      return token.name === 'ISSUE_PREFIX'
    })

    // Create references to configurable tokens for use in rules
    const BREAKING_CHANGE_TOKEN = this.breakingChangeToken
    const ISSUE_PREFIX = this.issuePrefixToken
